...
//             height: 34,
//             borderRadius: 17,
//             backgroundColor: isMine ? "rgba(255,255,255,0.22)" : "rgba(37,99,235,0.1)",
//             justifyContent: "center",
//             alignItems: "center",
//           }}
//         >
//           {isPlaying ? (
//             <Pause size={20} color={isMine ? "#FFF" : "#2563EB"} />
//           ) : (
//             <Play size={20} color={isMine ? "#FFF" : "#2563EB"} />
//           )}
//         </TouchableOpacity>
...
//           <View
//             style={{
//               backgroundColor: isMine ? "rgba(255,255,255,0.22)" : "#6B7280",
//               borderRadius: 12,
//               paddingHorizontal: 10,
//               paddingVertical: 3,
//             }}
...
//             <View style={{ marginRight: 8, marginBottom: 2 }}>
//               {otherUserAvatarUrl ? (
//                 <Image source={{ uri: otherUserAvatarUrl }} style={styles.avatarSmall} />
//               ) : (
//                 <View style={styles.avatarSmallFallback}>
//                   <Text style={{ color: "#FFF", fontSize: 10, fontWeight: "700" }}>
//                     {((otherUserName || "U")[0] || "U").toUpperCase()}
//                   </Text>
//                 </View>
...
//           <View style={[
//             styles.bubble,
//             isMine ? { backgroundColor: theme.myBubbleBg, borderBottomRightRadius: 6 } : { backgroundColor: theme.otherBubbleBg, borderBottomLeftRadius: 6 }
//           ]}>
//             <ActivityIndicator size="small" color={isMine ? "#FFF" : "#2563EB"} />
//           </View>
//         </View>
...
//       styles.msgContainer,
//       isBroadcast ? styles.broadcastContainer : (isMine ? styles.myContainer : styles.otherContainer),
//       isSelected && { backgroundColor: "rgba(37, 99, 235, 0.1)", borderRadius: 14 }
//     ]}>
//       <Animated.View
//         {...panResponder.panHandlers}
//         style={{
...
//               shadowColor: theme.myBubbleBg,
//             } : {
//               backgroundColor: theme.otherBubbleBg,
//               borderBottomRightRadius: 18,
//               borderBottomLeftRadius: 6,
//               borderWidth: theme.otherBorder ? 1 : 0,
//               borderColor: theme.otherBorder,
//             })
//           ]}
//         >
...
//               activeOpacity={0.8}
//               style={{
//                 backgroundColor: isMine ? "rgba(0,0,0,0.12)" : "rgba(37,99,235,0.06)",
//                 borderLeftWidth: 3,
//                 borderLeftColor: isMine ? "#FFF" : "#2563EB",
//                 padding: 7,
//                 borderRadius: 8,
...
//   // ── Keyboard listeners: track visibility AND keep the list pinned
//   // to the latest message the instant the keyboard opens/closes.
//   useEffect(() => {
//     const showEvent = Platform.OS === "ios" ? "keyboardWillShow" : "keyboardDidShow";
//     const hideEvent = Platform.OS === "ios" ? "keyboardWillHide" : "keyboardDidHide";
...
//               {/*
//                 Input area — single source of truth for bottom padding.
//                 - Keyboard open: tight padding (the keyboard itself provides the spacing).
//                 - Keyboard closed: respect the device safe-area inset (notch / home indicator).
//                 Same logic on iOS and Android now that softwareKeyboardLayoutMode is set to
//                 "pan" in app.json, which stops the OS from double-resizing the view.
//               */}
//               <View
//                 style={[
//                   styles.inputArea,
...
//   headerAvatarText: { color: "#FFF", fontSize: 16, fontWeight: "700" },
//   headerName: { fontSize: 16, fontWeight: "700", color: "#111827" },
//   onlineText: { fontSize: 12, color: "#22C55E", marginTop: 1, fontWeight: "600" },
//   offlineText: { fontSize: 11, color: "#9CA3AF", marginTop: 1 },
...
//   messageList: { padding: 12, paddingBottom: 10, paddingTop: 4 },
...
//   // ── Input area ──
//   // NOTE: paddingBottom is intentionally NOT set here — it's applied inline
//   // based on keyboard visibility / safe-area insets (see render code above).
//   // Keeping a single source of truth avoids the iOS/Android misalignment bug.
//   inputArea: {
//     flexDirection: "row",
//     alignItems: "flex-end",
//     paddingTop: 10,
...
//   // keyboard opens — synced to the keyboard's own native animation
//   // duration so the motion matches the OS exactly (no snap, no gap).
//   const inputBottomPad = useRef(new Animated.Value(Math.max(insets.bottom, 12))).current;
...
//   // ── Keyboard listeners: smoothly animate the input bar's bottom
//   // padding in sync with the keyboard's own show/hide animation
//   // (using its native `duration`, so there's no snap and no residual
//   // gap once it closes — this is the WhatsApp-style smooth behavior),
//   // and keep the list pinned to the latest message as it opens/closes.
//   // useEffect(() => {
//   //   if (Platform.OS === 'web') {
...
//   //         if (lastHeight - currentHeight > 120) {
//   //           setKeyboardVisible(true);
//   //           setTimeout(scrollBottom, 50);
//   //           setTimeout(scrollBottom, 150);
//   //         } else if (currentHeight - lastHeight > 120) {
//   //           setKeyboardVisible(false);
//   //         }
...
//   //   const animatePad = (toValue: number, duration?: number) => {
//   //     Animated.timing(inputBottomPad, {
//   //       toValue,
...
//   //     setKeyboardVisible(true);
//   //     animatePad(10, e?.duration);
//   //     scrollBottom();
//   //   });
//   //   const hideSubscription = Keyboard.addListener(hideEvent, (e: any) => {
//   //     setKeyboardVisible(false);
//   //     animatePad(Math.max(insets.bottom, 12), e?.duration);
//   //   });
...
//           // On web/PWA: read the safe-area-inset-top value injected at page load
//           // by the script in +html.tsx. This is 0 in browser and equals the phone's
//           // status-bar height when installed as a PWA (standalone mode).
//           ? (((typeof window !== 'undefined' && (window as any).__SAFE_AREA_TOP__) || 0) + 14)
//           : (insets.top > 0 ? insets.top + 8 : 14) 
//       }]}>
//         <TouchableOpacity onPress={() => router.back()} style={styles.backBtn}>
//           <Text style={styles.backIcon}>←</Text>
//         </TouchableOpacity>
...
//               scrollBottom();
//             } else {
//               // On web: only scroll when messages list grows (a new message arrived),
//               // not when the viewport shrinks because keyboard opened.
//               const currentLength = messages.length;
//               if (currentLength !== lastMessagesLengthRef.current) {
...
//           }}
//           onLayout={() => {
//             // Only scroll to bottom on the very first layout.
//             // On web, subsequent layout events are triggered by keyboard resize
//             // and would push messages to the top if we scroll here every time.
...
//               {/*
//                 Input area — single source of truth for bottom padding.
//                 On native: `inputBottomPad` is an Animated.Value that smoothly
//                 tweens between the safe-area inset (keyboard closed) and a tight
//                 10px (keyboard open), synced to the keyboard's own native
//                 animation duration via the listeners above.
//                 On web/PWA: CSS `interactive-widget=resizes-content` shrinks the
//                 viewport when keyboard opens, so the input stays anchored to the
//                 bottom automatically — no JS padding needed.
//               */}
//               <Animated.View
//                 style={[
//                   styles.inputArea,
...
//     flexDirection: "row",
//     alignItems: "center",
//     paddingTop: 14, // Base top padding; overridden inline with safe-area insets on native
//     paddingBottom: 14,
//     paddingHorizontal: 12,
//     backgroundColor: "#FFFFFF",
//     gap: 10,
...
//   // ── Input area ──
//   // NOTE: paddingBottom is intentionally NOT set here — it's driven by
//   // the `inputBottomPad` Animated.Value applied inline above. Keeping a
//   // single animated source of truth avoids the iOS/Android misalignment
//   // bug and gives a smooth WhatsApp-style transition instead of a snap.
//   inputArea: {
//     flexDirection: "row",
//     alignItems: "flex-end",
//     paddingTop: 10,
...
// Installed PWAs ("Add to Home Screen") run in standalone display mode.
// On many Android/iOS browsers the safe-area-inset script that sets
// window.__SAFE_AREA_TOP__ doesn't run in time (or at all) right after
// install, which leaves the header rendered with 0 top padding — i.e.
// drawn underneath the phone's own status bar/notch, so the back button
// is visually present but physically unreachable. This gives a sane
// minimum fallback in that situation.
...
export default function ChatScreen() {
  ...
  // IMPORTANT: scrollBottom is declared here, BEFORE any effect that
  // references it. Previously this was declared near the bottom of the
  // component (next to markSeen) while an earlier useEffect already
  // referenced it in its dependency array — that is a "use before
  // initialization" bug for `const`/useCallback bindings and is exactly
  // what caused the "Cannot access 'scrollBottom' before initialization"
  // crash, especially on web where the bundler doesn't hoist function
  // declarations the same way the Hermes/Android build did.
  const scrollBottom = useCallback(() => {
    setTimeout(() => {
      flatListRef.current?.scrollToEnd({
        animated: true,
  ...
  // Track the visual viewport height on web. On iOS Safari (including
  // standalone PWA) the meta-viewport value "interactive-widget=resizes-content"
  // is NOT supported, so the on-screen keyboard just overlays the page —
  // 100dvh doesn't shrink. By reading visualViewport.height and applying
  // it as an explicit container height we force the layout to fit the
  // visible area, keeping the message input above the keyboard.
  //
  // On Android Chrome this meta tag IS supported, so CSS already handles
  // the resize — applying a JS height there would cause a DOUBLE shrink
  // (= big gap between keyboard and input). We detect support at mount
  // and only apply the JS fallback where needed.
  const [webViewportHeight, setWebViewportHeight] = useState<number | null>(null);
  const needsViewportHeightHack = useRef(false);
  ...
  // and keep the list pinned to the latest message as it opens/closes.
  //
  // `isKeyboardVisible` is also consumed below (FlatList contentContainerStyle
  // and `maintainVisibleContentPosition`) — on web it was previously only
  // ever *set* and never *read*, which combined with
  // `maintainVisibleContentPosition` is what produced the "messages jump
  // to the top with a big blank gap" bug specifically on web/PWA when the
  ...
  useEffect(() => {
    if (Platform.OS === "web") {
      if (typeof window !== "undefined" && window.visualViewport) {
        ...
        // On iOS Safari the browser may scroll the <html>/<body>
        // when the keyboard opens. Pin scroll to 0 to prevent the
        // app from being pushed upward.
  ...
  const handleUpdateSettings = async (settings: any) => {
    ...
    if (success) {
      ...
      // Previously a failed update would silently no-op, which is almost
      // certainly why "unlock toggle off shows an alert and doesn't
      // unlock" — the modal shows its own success alert regardless of
      // what happened here. Surface the failure so the toggle visibly
      // reverts instead of looking stuck.
      Alert.alert("Error", "Could not update chat settings. Please try again.");
    }
  };
  ...
  const handleUpdateMemberSettings = async (settings: any) => {
    ...
    if (success) {
      ...
      Alert.alert("Error", "Could not update your chat settings. Please try again.");
    }
  };
  ...
  // PWA standalone header padding — computed once per render, cheap.
  const standalone = Platform.OS === "web" && isStandalonePWA();
  const webSafeAreaTop =
    Platform.OS === "web" && typeof window !== "undefined"
      ? (window as any).__SAFE_AREA_TOP__
      : undefined;
  // If we're an installed PWA but the safe-area script never reported a
  // value (or reported 0), fall back to a sane minimum so the header
  // (and its back button) isn't drawn underneath the phone's status bar.
  const headerTopPad =
    Platform.OS === "web"
      ? (webSafeAreaTop || (standalone ? 28 : 0)) + 14
      : (insets.top > 0 ? insets.top + 8 : 14);
  ...
  return (
    <View
      ...
      style={[
      styles.container,
      backgroundConfig.type === "color" && { backgroundColor: backgroundConfig.value },
      // On web (iOS Safari / iOS PWA only): use the tracked
      // visualViewport height so the layout shrinks when the keyboard
      // opens. This is NOT applied on Android Chrome where
      // interactive-widget=resizes-content already handles it via CSS.
      ...
      >
        <FlatList
          ...
          style={Platform.OS === "web" ? ({ scrollbarWidth: "none" } as any) : undefined}
          contentContainerStyle={[
            styles.messageList,
            // Trim the extra bottom padding while the keyboard is open on
            // web — this, combined with disabling maintainVisibleContentPosition
            // below, is what stops messages from appearing to "jump to the
            // top with a big blank gap" the moment you tap the input on
            // web/PWA.
            Platform.OS === "web" && isKeyboardVisible && { paddingBottom: 2 },
          ]}
          onContentSizeChange={() => {
            if (Platform.OS !== 'web') {
              scrollBottom();
          ...
          showsVerticalScrollIndicator={false}
          // maintainVisibleContentPosition fights with the visualViewport-driven
          // resize on RN Web (it tries to anchor scroll position to an index
          // while the browser is independently resizing the scroll container),
          // which is what produced the "jumps to top / big blank space" bug.
          // Native (iOS/Android) doesn't have that conflict, so keep it there.
              ...
              */}
              <Animated.View
                style={[
                  ...
                  {
                    paddingBottom: Platform.OS === 'web'
                      // When keyboard is open: minimal 4px pad so the
                      // input sits snug against the keyboard with almost
                      // no gap (WhatsApp-style).
                      // When keyboard is closed in PWA standalone: respect
                      // safe-area-inset-bottom (the home bar gesture area).
                      ? (isKeyboardVisible ? 4 : Math.max(insets.bottom, standalone ? 16 : 6))
                      : inputBottomPad,
                    // Tighten the top padding when keyboard is open too
                    ...(Platform.OS === 'web' && isKeyboardVisible && { paddingTop: 6 }),
                  },
                ]}
              >
                {!isRecording && (
